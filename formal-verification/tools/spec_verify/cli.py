...
@click.option("--lean", is_flag=True, default=False, help="Step 5-7: Lean translation, theorems, and proofs")
@click.option("--model", default="gpt-5.2", show_default=True, help="LLM model")
@click.option("--verbose", "-v", is_flag=True, default=False)
def main(file: str, issue: int | None, step2: bool, lean: bool, model: str, verbose: bool):
    """Analyze all functions in a C file, or work on a specific issue."""
    source_path = Path(file)

    from spec_verify.workflow import run_file_flow, run_issue_flow, run_step2_flow, run_lean_flow

    if issue:
        run_issue_flow(source_path, issue, model, verbose)
    elif lean:
        run_lean_flow(source_path, model, verbose)
    elif step2:
